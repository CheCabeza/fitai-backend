...
import { generateMealPlan, generateWorkoutPlan, getFitnessRecommendations } from '../utils/ai';
      ...
      if (req.user?.id) {
        ...
        if (supabase) {
          ...
          if (!userError && userDataFromDB) {
            userData = {
              age: userDataFromDB.date_of_birth
                ? new Date().getFullYear() - new Date(userDataFromDB.date_of_birth).getFullYear()
                : 25,
...
aiRoutes.get(
  ...
  async (req: AuthenticatedRequest, res: ApiResponseExpress): Promise<void> => {
    try {
      ...
      const { data: mealPlans, error: mealPlansError } = await mealPlansQuery.order('date', {
        ascending: true,
      });
      ...
      const { data: workoutPlans, error: workoutPlansError } = await workoutPlansQuery.order(
        'date',
        { ascending: true }
      );
      ...
      if (logs.length > 0) {
        ...
        const daysWithFood = new Set(foodLogs.map((log: any) => log.date.toDateString())).size;
        analysis.summary.averageCaloriesPerDay =
          daysWithFood > 0 ? Math.round(totalCalories / daysWithFood) : 0;

        // Calculate consistency score
        const totalDays = new Set(logs.map((log: any) => log.date.toDateString())).size;
      ...
      if (analysis.summary.consistencyScore < 70) {
        analysis.recommendations.push('Try to be more consistent with your daily logging');
      }
